...
_.extend(Slingshot.Directive.prototype, {
  ...
  getInstructions: function (method, file, meta) {
    ...
    check(instructions, {
      upload: String,
      //contextData: Match.OneOf(Object, null),
      download: String,
      postData: [{
        name: String,
...
Meteor.methods({
  ...
  "slingshot/uploadRequest": function (directiveName, file, meta) {
    ...
    var ins = directive.getInstructions(this, file, meta);

    //console.log(directive._directive.contextData());
    return _.extend(ins, directive._directive.contextData());
    //return ins;
  //  return _.extend(ins, this._directive.contextData);
  }
});
